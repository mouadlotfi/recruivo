...
<template>
    <Head :title="labels.create_job_title" />

    <AppLayout>
        <div class="space-y-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-stone-900 dark:text-white">{{ labels.create_job_title }}</h1>
                    <p class="mt-2 text-stone-600 dark:text-stone-400">{{ labels.create_job_subtitle }}</p>
                </div>
                <Link
            ...
            <div class="rounded-xl border border-stone-200/60 bg-white/80 p-8 backdrop-blur dark:border-stone-700/60 dark:bg-stone-900/60">
                <JobForm mode="create" :job="null" :categories="categories" :labels="labels" />
            </div>
        </div>
    </AppLayout>
</template>
