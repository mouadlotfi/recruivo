...
// Reserve space for the top-right actions only when a candidate sees them
// (mirrors the Blade pr-36/pr-16 padding so titles don't wrap).
...
const toggleSaved = () => {
    if (isDemoCandidate.value) return
    const url = localeUrl(`/candidate/saved-jobs/${props.job.id}`)
    // Controller redirects back with a flash; preserveState keeps the page
    // mounted and the redirected visit re-serializes is_saved.
    if (props.job.is_saved) {
        router.delete(url, { preserveState: true, preserveScroll: true })
    } else {
        router.post(url, {}, { preserveState: true, preserveScroll: true })
    }
}
</script>
    ...
    <div class="group relative h-full rounded-xl border border-stone-200/60 bg-white/80 p-6 shadow-sm transition hover:shadow-md dark:border-stone-700/60 dark:bg-stone-900/60">
        <div v-if="isCandidate" class="absolute right-3 top-3 z-10 flex items-center gap-2">
            ...
            <button
                ...
                class="relative z-10 inline-flex h-11 w-11 items-center justify-center rounded-full transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500"
                :class="job.is_saved
        ...
        <div class="flex items-start gap-4" :class="actionReserve">
            ...
            <div class="min-w-0 flex-1">
                ...
                <div class="mt-2 flex flex-wrap gap-2 text-xs text-stone-500 dark:text-stone-400">
                    ...
                    <Link
                        ...
                        class="relative z-10 rounded-full px-2 py-0.5 font-medium transition"
                ...
                <p v-if="salaryRange" class="mt-2 text-sm font-medium text-stone-700 dark:text-stone-300">
                    {{ salaryRange }}
                </p>

                <p
                    v-if="job.is_closing_soon"
                    class="mt-2 inline-flex items-center gap-1 rounded-full bg-amber-100 px-2.5 py-0.5 text-xs font-medium text-amber-700 dark:bg-amber-500/10 dark:text-amber-300"
                >
